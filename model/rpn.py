...
class RegionProposalNetwork(tf.keras.layers.Layer):
    ...
    def call(self, feature_map, gt_boxes=None):
        ...
        if gt_boxes is not None:
            ...
            losses['rpn_class_loss'] = rpn_class_loss(cls_flat, anchors_flat, gt_boxes)
            losses['rpn_bbox_loss']  = rpn_bbox_loss(reg_flat, anchors_flat, gt_boxes)
    ...
    def _generate_proposals(self, cls_logits, bbox_preds):
        ...
        scores = tf.nn.softmax(
            tf.reshape(cls_logits, [B, -1, 2]), axis=-1
        )[...,1]  # [B,N]
        deltas = tf.reshape(bbox_preds, [B, -1, 4])  # [B,N,4]

        # anchors flat
        anchors = self._generate_anchors(Hf, Wf)
        anchors = tf.reshape(anchors, [-1,4])  # [N,4]
